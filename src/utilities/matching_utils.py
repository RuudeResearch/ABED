...
import torch
import warnings
from utilities.jaccard_overlap import jaccard_overlap
from utilities.encode import encode


def match_events_localization_to_default_localizations_old(localizations_default, events, threshold_overlap):
    batch = len(events)
...
def match_events_localization_to_default_localizations(localizations_default,
                                                       events,
                                                       threshold_overlap):
    batch = len(events)
    ...
    for batch_index in range(batch):
        number_of_events = events[batch_index].shape[0]
        # If no event add default value to predict (will never be used anyway)
        # And class 0 == backgroung
        if number_of_events == 0:
            localizations_target[batch_index][:, :] = torch.FloatTensor(
                [[0, 1]]).expand_as(localizations_default)
            classifications_target[batch_index] = torch.zeros(localizations_default.size(0))
            continue
        ...
        best_truth_idx.squeeze_(0)
        best_truth_overlap.squeeze_(0)
        if number_of_events != len(set(best_truth_idx.numpy())):
            warnings.warn("Some true events are not matched !! Use a better default events space!")
            # print("lol")
        localization_match = localizations_truth[best_truth_idx]
        localization_target = encode(localization_match, localizations_default)
        classification_target = classifications_truth[best_truth_idx] + 1  # Add class 0!
