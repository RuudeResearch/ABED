...
def jaccard_overlap(localizations_a, localizations_b):
    ...
    Parameters:
    -----------
    localizations_a: tensor, shape (n_loc_a, 2)
        An array of localizations
    localizations_b: tensor, shape (n_loc_b, 2)
        An array of localizations
    ...
    lentgh_a = (localizations_a[:, 1] - localizations_a[:, 0]).unsqueeze(
        1).expand(A, B)
    lentgh_b = (localizations_b[:, 1] - localizations_b[:, 0]).unsqueeze(
        0).expand(A, B)
